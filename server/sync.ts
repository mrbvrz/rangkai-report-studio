...
export function syncSource(sourceId: number) {
  const source = db.prepare("SELECT * FROM project_sources WHERE id = ?").get(sourceId) as
  ...
  for (const filePath of markdownFiles(folder)) {
    ...
    const file = db
      .prepare("SELECT * FROM source_files WHERE source_id = ? AND file_path = ?")
      .get(sourceId, filePath) as
      { id: number; report_id?: number; file_mtime: number; status: string } | undefined
    if (!file) {
      const report = db
        .prepare("SELECT id, source_mtime FROM reports WHERE project_id = ? AND source_path = ?")
        .get(source.project_id, filePath) as { id: number; source_mtime: number } | undefined
...
export function syncWatchedSources() {
  ...
  syncing = true
  try {
    const sources = db.prepare("SELECT id FROM project_sources WHERE is_watching = 1").all() as {
      id: number
    }[]
    for (const source of sources) {
      try {
        ...
        ).run(error instanceof Error ? error.message : "Sinkronisasi gagal.", source.id)
      }
    }
  } finally {
    syncing = false
