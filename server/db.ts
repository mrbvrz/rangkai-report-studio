...
function initialize(database: BetterSqlite3.Database) {
  ...
  database
    .prepare(
      "UPDATE reports SET project_id = ? WHERE project_id IS NULL AND source_path IS NULL",
    )
...
export function changeDatabasePassphrase(
...
export const db = new Proxy({} as BetterSqlite3.Database, {
  get: (_target, key) => {
    const value = (
      requireConnection() as unknown as Record<PropertyKey, unknown>
    )[key];
    return typeof value === "function"
      ? value.bind(requireConnection())
      : value;
  },
});
