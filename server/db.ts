...
function requireConnection() {
  if (!connection) throw Object.assign(new Error('Database terkunci.'), { code: 'DATABASE_LOCKED' })
  return connection
}
...
function initialize(database: BetterSqlite3.Database) {
  ...
  database.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_reports_project_source ON reports(project_id, source_path) WHERE source_path IS NOT NULL')
  const project = database.prepare('SELECT id FROM projects ORDER BY id LIMIT 1').get() as { id: number } | undefined
  if (!project) database.prepare('INSERT INTO projects (name, description, color) VALUES (?, ?, ?)').run('Project Utama', 'Project default untuk laporan baru.', '#6c8f58')
  const defaultProject = database.prepare('SELECT id FROM projects ORDER BY id LIMIT 1').get() as { id: number }
  database.prepare('UPDATE reports SET project_id = ? WHERE project_id IS NULL AND source_path IS NULL').run(defaultProject.id)
  const template = database.prepare('SELECT id FROM templates LIMIT 1').get()
  if (!template) database.prepare('INSERT INTO templates (name, description, content, is_default) VALUES (?, ?, ?, 1)').run('Laporan Profesional', 'Format ringkas dengan ringkasan, aktivitas, dan lampiran.', '# {{title}}\n\n## Ringkasan Eksekutif\n\n{{summary}}\n\n## Rincian Aktivitas\n\n{{daily_reports}}\n\n## Lampiran\n\n{{attachments}}\n')
}
...
export function lockDatabase() { connection?.close(); connection = null; activePassphrase = null }
export function databaseStatus() { return connection ? 'unlocked' : 'locked' }
export function databasePassphrase() { if (!activePassphrase) throw new Error('Database terkunci.'); return activePassphrase }
export const db = new Proxy({} as BetterSqlite3.Database, { get: (_target, key) => {
...
export function serializeReport(row: Record<string, unknown>) { return { ...row, tags: JSON.parse(String(row.tags || '[]')) } }
