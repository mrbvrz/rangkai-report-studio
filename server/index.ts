...
app.use("/api", (req, res, next) => {
  ...
  res
    .status(423)
    .json({ message: "Database terkunci.", code: "DATABASE_LOCKED" });
});
...
app.put("/api/database/passphrase", (req, res) => {
  try {
    ...
    res.status(400).json({
      message:
        error instanceof Error ? error.message : "Passphrase tidak dapat diubah.",
    });
  }
});
...
function decryptAttachment(filePath: string) {
  ...
  if (payload.subarray(0, 5).toString() !== "RGAT1")
    throw new Error("Lampiran tidak terenkripsi.");
  const decipher = crypto.createDecipheriv(
    "aes-256-gcm",
    attachmentKey(),
    payload.subarray(5, 17),
  );
  decipher.setAuthTag(payload.subarray(17, 33));
  return Buffer.concat([
    decipher.update(payload.subarray(33)),
    decipher.final(),
  ]);
}
...
function createBackup() {
  ...
  const files = (tables.attachments as { filename: string }[]).flatMap(
    (attachment) => {
...
function clearWorkspace(all: boolean) {
  const attachmentFiles = db
    .prepare("SELECT filename FROM attachments")
...
function attachmentsFor(reportId: number) {
  return db
    .prepare("SELECT * FROM attachments WHERE report_id = ? ORDER BY id")
    .all(reportId);
}
...
app.post("/api/settings/restore", backupUpload.single("backup"), (req, res) => {
  try {
    if (!req.file)
      return res.status(400).json({ message: "File backup wajib dipilih." });
...
app.get("/api/dashboard-summary", (_req, res) => {
  ...
  const attachments = db
    .prepare("SELECT COUNT(*) AS value FROM attachments")
    .get() as { value: number };
  const months = db
    .prepare("SELECT COUNT(DISTINCT month) AS value FROM monthly_reports")
    .get() as { value: number };
  const projects = db
    .prepare("SELECT COUNT(*) AS value FROM projects")
    .get() as { value: number };
  res.json({
...
app.get("/api/dashboard-analytics", (_req, res) => {
  ...
  const projects = Array.from(
    ...
    values: periods.map(
      (period) =>
        projectRows.find(
          (row) => row.id === project.id && row.period === period,
        )?.count || 0,
    ),
  }));
  const start = new Date(year, 0, 1);
  const daysInYear = Math.round(
    (new Date(year + 1, 0, 1).getTime() - start.getTime()) / 86_400_000,
  );
...
app.get("/api/filesystem/directories", (req, res) => {
  try {
    ...
    res
      ...
      .json({
        message:
          error instanceof Error ? error.message : "Folder tidak dapat dibaca.",
      });
  }
});
...
app.delete("/api/projects/:id", (req, res) => {
  ...
  if (count.count <= 1)
    return res
      .status(400)
      .json({ message: "Minimal satu project harus tersedia." });
  const project = db
  ...
  db.transaction(() => {
    db.prepare("UPDATE reports SET project_id = NULL WHERE project_id = ?").run(
      req.params.id,
    );
    db.prepare(
      "UPDATE monthly_reports SET project_id = NULL WHERE project_id = ?",
    ).run(req.params.id);
...
app.post("/api/projects/:id/sources", (req, res) => {
  ...
  if (!folder || !fs.existsSync(folder) || !fs.statSync(folder).isDirectory())
    return res
      .status(400)
      .json({ message: "Folder tidak ditemukan atau tidak dapat dibaca." });
  try {
    const result = db
      .prepare(
    ...
    res
      ...
      .json({
        message:
          error instanceof Error ? error.message : "Gagal menambahkan sumber.",
      });
  }
});
...
app.post("/api/source-files/actions", (req, res) => {
  ...
  try {
    const results = db.transaction(() =>
      fileIds.map((id) => applySourceFile(Number(id), action)),
    )();
...
app.get("/api/reports", (req, res) => {
  ...
  const where = [
    ...
    .filter(Boolean)
    .join(" AND ");
  const params = [month || null, projectId || null].filter(
    (value) => value !== null,
  );
  const rows = db
    .prepare(
...
app.post("/api/reports", (req, res) => {
  ...
  if (!projectId || !title || !reportDate || !content)
    return res
      .status(400)
      .json({ message: "Project, judul, tanggal, dan isi wajib diisi." });
  const result = db
    .prepare(
      ...
      res
        ...
        .json({
          message:
            error instanceof Error
              ? error.message
              : "Lampiran tidak dapat dienkripsi.",
        });
    }
  },
);
...
app.get("/api/templates", (_req, res) =>
  res.json(
    db.prepare("SELECT * FROM templates ORDER BY is_default DESC, name").all(),
  ),
);
...
function compileMonth(
  ...
  const template = templateId
    ...
    : (db
        .prepare("SELECT * FROM templates ORDER BY is_default DESC, id LIMIT 1")
        .get() as { id: number; content: string });
  if (!reports.length)
    throw new Error("Belum ada laporan harian pada periode ini.");
  ...
  const daily = reports
    .map(
      (report) =>
        `### ${report.report_date} — ${report.title}\n\n${report.content}`,
    )
    .join("\n\n");
...
app.get("/api/monthly", (_req, res) =>
  res.json(
    db
      .prepare("SELECT * FROM monthly_reports ORDER BY month DESC, id DESC")
      .all(),
  ),
);
...
app.get("/api/monthly/:id/export.md", (req, res) => {
  const row = db
    ...
    .get(req.params.id) as { title: string; content: string } | undefined;
...
app.listen(port, () =>
  console.log(`Rangkai API aktif di http://localhost:${port}`),
);
setInterval(() => {
