...
export function Reports() {
  const [reports, setReports] = useState<Report[]>([]),
    ...
    [sort, setSort] = useState<SortOption>("date-desc"),
    [view, setView] = useState<ViewMode>("grid"),
  ...
  const filtered = useMemo(() => {
    ...
    return reports
      .filter(
        (report) =>
          (!projectId || report.project_id === Number(projectId)) &&
          (!status || report.status === status) &&
          `${report.title} ${report.content} ${report.tags.join(" ")}`
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      {deletingReport && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[#20251f]/45 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-md p-6">
            <div className="mb-4 grid h-11 w-11 place-items-center rounded-xl bg-[#fff0ed] text-[#b23c27]">
              <Trash2 size={19} />
            </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button
                $variant="danger"
                disabled={busy}
                onClick={() => void remove(deletingReport.id)}
              >
                {busy ? "Menghapus…" : "Hapus laporan"}
              </Button>
            </div>
          </Card>
        </div>
      ...
      <div className="relative z-20 mb-5 p-0">
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          <div className="relative min-w-0 flex-1">
            ...
            />
            <kbd className="pointer-events-none absolute right-3 top-2.5 hidden rounded-md border border-[#e0e3dc] bg-[#f7f8f5] px-2 py-1 text-[10px] font-medium text-[#8b9188] sm:block">
              ⌘ K
            </kbd>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              <button
                ...
                aria-haspopup="menu"
                className={`flex h-11 items-center gap-2 rounded-xl border px-3 text-sm font-medium transition ${filterOpen || projectId || status || sort !== "date-desc" ? "border-[#b9cfac] bg-[#eef5e9] text-[#49633f]" : "border-[#dfe2da] bg-white text-[#4f574c]"}`}
              >
                <Filter size={16} /> Filter{" "}
                {(projectId || status || sort !== "date-desc") && (
                  <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#6c8f58] px-1 text-[10px] text-white">
                    {Number(Boolean(projectId)) +
                      Number(Boolean(status)) +
                      Number(sort !== "date-desc")}
                  </span>
                )}
                <ChevronDown
                  size={15}
              ...
              {filterOpen && (
                <div
                  role="menu"
                  className="absolute right-0 top-12 w-72 rounded-2xl border border-[#e1e4dc] bg-white p-4 shadow-xl"
                >
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-xs font-medium uppercase tracking-[.12em] text-[#858b82]">
                      Filter laporan
                    </p>
                    <button
                      type="button"
                  ...
                  <div className="space-y-3">
                    <label className="block text-xs font-medium text-[#545c50]">
                      Project
                      <Select
                        value={projectId}
            ...
            <div
              className="flex rounded-xl bg-[#f1f3ee] p-1"
              role="group"
            ...
            >
              <button
                ...
                className={`flex h-9 items-center gap-2 rounded-lg px-3 text-xs font-medium transition ${view === "grid" ? "bg-white text-[#293126] shadow-sm" : "text-[#7b8277] hover:text-[#293126]"}`}
              >
              ...
              <button
                ...
                className={`flex h-9 items-center gap-2 rounded-lg px-3 text-xs font-medium transition ${view === "list" ? "bg-white text-[#293126] shadow-sm" : "text-[#7b8277] hover:text-[#293126]"}`}
              >
                <List size={16} /> List
              </button>
            </div>
      ...
      {filtered.length ? (
        <>
              ...
              {visibleReports.map((report) =>
                view === "grid" ? (
                  <Card
                    ...
                    key={report.id}
                    style={{ borderColor: `${report.project_color}55` }}
                    className="group p-5 transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:shadow-lg"
                  >
                    <div className="mb-5 flex items-start justify-between">
                      ...
                      <button
                        ...
                        className="rounded-lg p-2 text-[#a2a69f] opacity-0 transition hover:bg-red-50 hover:text-red-600 group-hover:opacity-100 group-focus-within:opacity-100"
                      >
                        <Trash2 size={15} />
                      </button>
                    </div>
                    <Link to="/reports/$reportId" params={{ reportId: String(report.id) }}>
                      <div className="mb-2 flex items-center gap-2">
                        <span
                          ...
                          style={{ background: report.project_color }}
                        />
                        <span className="truncate text-[10px] font-medium uppercase tracking-[.1em] text-[#858b82]">
                          {report.project_name}
                        </span>
                      ...
                      <div className="mt-5 flex items-center justify-between border-t border-[#eef0ea] pt-4">
                        <span className="text-xs font-medium text-[#868c83]">
                          {displayDate(report.report_date)}
                        </span>
                        <span className="flex items-center gap-1.5 text-xs text-[#868c83]">
                          <Image size={14} /> {report.attachments.length}
                        </span>
                      </div>
                    </Link>
                  </Card>
                  ...
                  <Card
                    ...
                    key={report.id}
                    className="group flex items-center transition-[transform,box-shadow] duration-300 hover:-translate-y-0.5 hover:shadow-md gap-3 p-3 sm:gap-4 sm:p-4"
                  >
                    <div
                    ...
                    <Link
                      ...
                      className="min-w-0 flex-1 sm:grid sm:grid-cols-[minmax(180px,1fr)_minmax(120px,180px)_110px_70px] sm:items-center sm:gap-4"
                    >
                      ...
                      <p className="mt-1 truncate text-xs text-[#747a71] sm:mt-0">
                        {report.content.replace(/[#*_`]/g, "")}
                      </p>
                      <span className="mt-1 block text-xs font-medium text-[#868c83] sm:mt-0">
                        {displayDate(report.report_date)}
                      </span>
                      <span className="mt-1 flex items-center gap-1.5 text-xs text-[#868c83] sm:mt-0">
                        <Image size={14} /> {report.attachments.length}
                      </span>
                    </Link>
                    <button
                      ...
                      onClick={() => setDeletingReport(report)}
                      aria-label={`Hapus ${report.title}`}
                      className="shrink-0 rounded-lg p-2 text-[#a2a69f] opacity-100 transition hover:bg-red-50 hover:text-red-600 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100"
                    >
                      <Trash2 size={15} />
                    </button>
                  </Card>
          ...
          <div className="mt-5 flex flex-col items-center justify-between gap-3 px-1 py-3 sm:flex-row">
            <div className="flex items-center gap-3 text-xs text-[#747a71]">
              <span>
                Menampilkan {firstResult}–{lastResult} dari {filtered.length}
              </span>
        ...
        <EmptyState
          ...
          title={reports.length ? "Tidak ditemukan" : "Belum ada laporan"}
          body={
            reports.length
              ? "Coba ubah kata kunci atau filter yang dipilih."
              : "Impor Markdown atau tulis laporan harian pertama Anda."
          }
          action={
            !reports.length && (
              <Link to="/reports/new">
                <Button>Mulai menulis</Button>
              </Link>
            )
          }
        />
