...
const normalizeRecovery = (value: string) =>
...
export function SecurityProvider({ children }: { children: ReactNode }) {
  ...
  async function createProtection(passphrase: string, encrypted: boolean) {
    ...
    const root = bytes(32);
    const recoveryRaw = Array.from(bytes(20), (part) =>
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            <span className="inline-flex items-center justify-center gap-2">
              {busy ? (
                setup ? "Menyiapkan…" : "Memverifikasi…"
              ) : recovery ? (
                <>
          ...
          >
            {recovery
              ? "Kembali ke passphrase"
              : "Lupa passphrase? Gunakan recovery code"}
          </button>}
        </div>
      </motion.div>
    </motion.div>
...
export function useSecurity() {
  ...
  if (!context)
    throw new Error("useSecurity harus digunakan di dalam SecurityProvider.");
  return context;
}
