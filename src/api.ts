export async function api<T>(url: string, options?: RequestInit): Promise<T> {
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try { body = JSON.parse(text) as Record<string, unknown> | T }
    catch {
    ...
    if (!response.ok) throw new Error(String((body as Record<string, unknown>).message || 'Terjadi kesalahan.'))
    return body as T
  }
  throw new Error('Server lokal belum siap.')
}

export const displayDate = (date: string) => new Intl.DateTimeFormat('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(`${date}T00:00:00`))
