    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(
          new Response('<script type="module"></script>', { status: 200 }),
        )
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ reports: 3 }), { status: 200 }),
        ),
    );
    await expect(
      api<{ reports: number }>("/dashboard-summary"),
    ).resolves.toEqual({ reports: 3 });
